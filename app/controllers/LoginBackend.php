...
class LoginBackend extends CI_Controller{
    ...
    public function auth()
    {
     ...
     $user = $this->db->select('u.id, u.rolecode, g.rolename, u.username,  u.email, u.mobile,  u.disabled, u.instcode,i.instname ')
     ->from('sysusers u')
     ->where("u.email='{$email}' ")
     ->where("u.password='{$password_check}' ")
     ->join('sysroles g', 'g.rolecode = u.rolecode', 'left')
     ->join('institutions i', 'i.instcode = u.instcode', 'left')
     ->get();
    ...
    $user_data['instcode']   = $row->instcode;
    $user_data['instname']   = $row->instname;
