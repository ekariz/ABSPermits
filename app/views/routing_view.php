...
$select_route_to = '<select id="route_to" name="route_to" class="form-control"  >';
foreach($approvalsteps as $stepno=>$stepname){
 $select_route_to .='<option value="'.$stepno.'">'.$stepname.'</option>';
}
$select_route_to .= '</select>';
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title" id="h3_title" ><div id="div_titlex">Routing </div></h3>
            </div>
...
-->
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="btnRoute" onclick="workflow.init_approve()" class="btn btn-success"><i class="fa fa-code-fork"></i> Route</button>
                <button type="button" id="btnDispproveInit" onclick="workflow.init_disapprove()" class="btn btn-danger"><i class="fa fa-history"></i> Archive</button>
...
<div class="modal fade" id="modal_form_approve" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title"><?php echo $approval_name; ?> </h3>
            </div>

            <div class="modal-bodyx formx">

                <form action="<?php echo $this->router->class;?>/approve"   id="form-route" class="form-horizontal" method="post" enctype="multipart/form-data" >

                
                <div class="form-group">
                <label class="col-sm-2 control-label" for="route_to">Route To</label>
                 <div class="col-sm-10">
                   <?php  echo $select_route_to; ?>
                 </div>
                </div>
                
                <div class="form-group">
                <label class="col-sm-2 control-label" for="consult_with">Comments</label>
                 <div class="col-sm-10">
                   <textarea type="text" class="form-control  " name="comments-route" id="comments-route"   placeholder="Comment..."  ></textarea>
                 </div>
                </div>
            ...
            <div class="modal-footer">
                <button type="button" id="btnModalApprove" onclick="workflow.approve()" class="btn btn-success"><i class="fa fa-check"></i> Route</button>
                <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
...
var workflow ={
 ...
 approve:function (){
  ...
  var comments = $('#comments-route').val();
  if(route_to===''){
    swal({ text: 'Select Route to Institution', icon: "warning"});
   return;
  }else if(comments===''){
  ...
  $.SmartMessageBox({
   title : "Confirm",
   content : "Route Application No. <font color='green'>"+appno+"</font>?",
   buttons : '[No][Yes]'
  },function(bp) {
  if (bp === "Yes") {
   $('#btnRoute').attr('disabled',true);
   ...
   $.ajax({
    ...
    type: "POST",
    data: $('#form-data, #form-route').serialize() + '&comments=' + comments,
    dataType: "JSON",
    success: function(data)
    {
       if(data.success==1) {
    ...
    error: function (jqXHR, textStatus, errorThrown)
   {
    $('#btnRoute').attr('disabled',false);
    $('#btnDispprove').attr('disabled',false);
    swal({ text: 'Error routing', icon: "error"});
   }
  });
  }
 });
